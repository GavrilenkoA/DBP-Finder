...
def output_list():
    ...
    while next:
        ...
        for i, item in enumerate(payload["results"]):
            accession.append(parse_column(item["metadata"]["accession"], 'metadata.accession'))
            source_database.append(parse_column(item["metadata"]["source_database"], 'metadata.source_database'))
            name.append(parse_column(item["metadata"]["name"], 'metadata.name'))
            source_organism.append(parse_column(item["metadata"]["source_organism"]["scientificName"],
                                                'metadata.source_organism.scientificName'))
            length.append(parse_column(item["metadata"]["length"], 'metadata.length'))

        # Don't overload the server, give it time before asking for more
        if next:
            sleep(1)

    df = pd.DataFrame({"accession": accession, "source_database": source_database, "name": name, "source_organism": source_organism, "length": length})
    df.to_csv(f"{output_data}.csv", index=False)
