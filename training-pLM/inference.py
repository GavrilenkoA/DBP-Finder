...
from dataset import (
    dataloader_prepare,
    InferenceDataset as IF_Dataset,
    SequenceDatasetWithID,
)
from utils import (
    InferenceDataset,
    SequenceDataset,
...
def ankh_head_inference(test_data):
    ...
    models = load_models(
        prefix_name="checkpoints/models/DBP-Finder_", config_path="config.yml"
    )
    ...
    predictions_df = inference_ensemble_based_on_threshold(
        models, inference_dataloader, thresholds, DEVICE
    )
    # predictions_df = inference(models, inference_dataloader, DEVICE)
    predictions_df.to_csv(
        "../data/not_annotated/dbo-finder-v2-predictions.csv", index=False
    )
