...
from train_ankh_utils import ensemble_predict
...
task = Task.init(
    ...
    task_name=f"ff_ankh_train_p3-{test_data}-inference",
    output_uri=False,
)
logger = Logger.current_logger()


def load_thresolds(model_name="lora_training_train_p3"):
...
def evaluate_on_test(test_data: str) -> None:
    ...
    models = load_models()
    thresholds = load_thresolds(model_name="1_epoch_training_overall")
...
def ankh_inference(test_data):
    # models, tokenizer = load_lora_models(prefix_name="checkpoints/Ankh_full_finetuned_", num_models=3)
    ...
    dataloader = dataloader_prepare(test_df, tokenizer)
    ...
    predictions_df.to_csv(csv_buffer, index=False)
    csv_buffer.seek(0)
    task.upload_artifact(name=f"ff_ankh_train_{test_data}", artifact_object=csv_buffer)
    task.close()
